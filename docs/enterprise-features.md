...
|---------|-------------|------|------|-------------|-------------------------------------------|
| Email/password auth | Secure login with hashed passwords and session tokens | 1 | team | integration-runner audit, Opus review | Supabase Auth `signInWithPassword`; FastAPI verifies via `supabase.auth.getUser()` |
| Google Workspace SSO | OAuth 2.0 login restricted to organization domain | 1 | production | integration-runner audit, Opus review | Supabase Auth `signInWithOAuth({ provider: 'google' })`; restrict domain in Supabase Auth settings |
| MFA (TOTP + SMS) | Multi-factor authentication with TOTP primary and SMS fallback | 1 | production | integration-runner audit, Opus review | Supabase Auth MFA APIs: `mfa.enroll()`, `mfa.challenge()`, `mfa.verify()` |
| RBAC (4 roles minimum) | Role-based access: Admin, Manager, Member, Viewer | 1 | production | integration-runner audit, Opus review | `roles` + `user_roles` tables; custom JWT claims via Supabase hook; FastAPI `Depends(require_role("admin"))` |
...
| Account lockout | Lock account after N failed login attempts | 1 | production | integration-runner audit, Opus review | `login_attempts` counter in profiles table; Supabase Auth hook or FastAPI middleware to check before auth |
| Invite-only onboarding | No open self-registration; admin sends invite with signed token | 1 | production | integration-runner audit, Opus review | `invitations` table (email, role_id, token, expires_at); registration validates invite token |
...
|---------|-------------|------|------|-------------|----------------|
| Health endpoint | `GET /health` returns service + DB + dependency status | 1 | team | integration-runner audit, Opus review | FastAPI route: ping Supabase, check DB, return `{"status": "healthy", "db": "ok", "auth": "ok"}` |
...
| Rate limiting | Rate limits on all API endpoints, stricter on auth | 1 | production | post-edit-enforce, integration-runner audit, Opus review | `slowapi` library with `Limiter` on FastAPI app; per-route decorators `@limiter.limit("10/minute")` |
...
| Feature | Description | Tier | Gate | Enforcement | Implementation |
|---------|-------------|------|------|-------------|----------------|
| Pagination (all lists) | Every list endpoint and UI table uses pagination — never unbounded | 1 | production | post-edit-enforce, integration-runner audit, Opus review | Cursor-based pagination; FastAPI `limit`/`cursor` params; Vue `usePagination()` composable |
...
| CSP headers | Content Security Policy preventing XSS | 1 | production | integration-runner audit, Opus review | Nuxt `security` module or custom middleware setting `Content-Security-Policy` header |
| CORS allowlist | Explicit origin allowlist, no wildcard | 1 | production | integration-runner audit, Opus review | FastAPI `CORSMiddleware` with `allow_origins=["https://yourdomain.com"]` |
...
| Health check | Service + DB + dependency status endpoint | 1 | team | integration-runner audit | (Same as Resilience section — `/health` endpoint) |
